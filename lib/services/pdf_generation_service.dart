...
// Analysis fields and their checkmarks (Page 4)
  ...
  Future<Uint8List> generateOfficialPdf({
    ...
    final pdfType = specificPdfType ?? 'raport_iscir';

    switch (pdfType) {
      case 'raport_iscir':
        return await _generateRaportIscirPdf(form, client, formData);
      case 'anexa4':
        return await _generateAnexa4Pdf(form, client, formData);
      default:
        return await _generateRaportIscirPdf(form, client, formData);
    }
  }
  ...
  // Methods for generating Anexa 4 PDF
  Future<Uint8List> _generateAnexa4Pdf(ISCIRForm form, Client client, Map<String, dynamic> formData) async {
    final pdf = pw.Document();

    try {
      // Load the Anexa 4 template image (you'll need to add this to assets/templates/)
      final ByteData templateData = await rootBundle.load('assets/templates/anexa4_template.png');
      final Uint8List templateBytes = templateData.buffer.asUint8List();
      final pw.MemoryImage templateImage = pw.MemoryImage(templateBytes);
      ...
      print('Anexa 4 PDF - Image: ${imageWidth}x$imageHeight, PDF: ${pdfWidth}x$pdfHeight, Scale: ${scaleX}x$scaleY');
          ...
          build: (pw.Context context) {
            return pw.Stack(
              children: [
                ...
                // Overlay form data at mapped coordinates
                ..._buildAnexa4FieldOverlays(formData, client, ttf, scaleX, scaleY, form),
              ],
            );
          },
        ),
      ...
      print('Error generating Anexa 4 PDF: $e');
      rethrow;
    }
  }

  List<pw.Widget> _buildAnexa4FieldOverlays(
      Map<String, dynamic> formData,
      Client client,
      pw.Font font,
      double scaleX,
    ...
    List<pw.Widget> overlays = [];
    final fieldPositions = _getAnexa4FieldPositions();

    print('Building Anexa 4 overlays...');
    print('Available field positions: ${fieldPositions.keys.length}');
    print('Form data keys: ${formData.keys.toList()}');

    // === AUTO-GENERATE REGISTRY NUMBER ===
    // You can auto-generate this or use form data
    final registryNumber = formData['registry_number'] ?? '${DateTime.now().year}-${form.id ?? 'NEW'}';
    ...
    // === CLIENT INFORMATION (from client data) ===
    _addTextOverlay(
      overlays,
      'denumire_utilizator',
      client.name,
    ...
    _addTextOverlay(
      overlays,
      'localitate_judet',
      client.address, // Use client address as locality
      fieldPositions,
      font,
      scaleX,
      scaleY,
    ...
    // Block/Stair/Floor/Apartment - this might be empty for most cases
    _addTextOverlay(
      overlays,
      'bl_sc_et_ap',
      '', // Usually empty for industrial equipment
      fieldPositions,
      font,
      scaleX,
      scaleY,
    ...
    // === OPERATION CHECKMARKS ===
    _addAnexa4OperationText(overlays, formData, fieldPositions, font, scaleX, scaleY);

    // === EQUIPMENT CHARACTERISTICS ===
    // Use data from the main form
    _addTextOverlay(
      overlays,
      'tip_aparat',
      _getFieldValue(formData, 'producator') + (formData['tip'] != null ? ' ${formData['tip']}' : ''),
      fieldPositions,
      font,
      scaleX,
      scaleY,
    ...
    // Combine power and other parameters
    final parametri = <String>[];
    if (formData['putere'] != null) parametri.add('${formData['putere']} kW');
    if (formData['tip_combustibil'] != null) parametri.add(formData['tip_combustibil'].toString());
    ...
    _addTextOverlay(
      overlays,
      'nr_fabricatie_an',
      formData['serie_an_fabricatie']?.toString() ?? '',
      fieldPositions,
      font,
      scaleX,
      scaleY,
    ...
    _addTextOverlay(
      ...
      '${formData['report_no'] ?? form.reportNumber} / ${formData['today_date'] ?? _formatDate(form.reportDate)}',
      fieldPositions,
      font,
      scaleX,
      scaleY,
    ...
    // === EQUIPMENT BOOKLET (usually empty initially) ===
    _addTextOverlay(
      overlays,
      'livret_aparat_nr_data',
      '', // This is usually filled later by authorities
      fieldPositions,
      font,
      scaleX,
      scaleY,
    ...
    // === NEXT VERIFICATION DATE (2 years from now) ===
    final nextVerificationDate = form.reportDate.add(const Duration(days: 365 * 2));
    _addTextOverlay(
      overlays,
      'scadenta_urmatoare_verificare',
      _formatDate(nextVerificationDate),
      fieldPositions,
      font,
      scaleX,
      scaleY,
    ...
    if (formData['aparat_admis'] == 'admis') {
      observations.add('Aparat admis');
    } else if (formData['aparat_admis'] == 'respins') {
      observations.add('Aparat respins');
    }
    ...
    print('Created ${overlays.length} overlays for Anexa 4');
    return overlays;
  }

  void _addAnexa4OperationText(
      List<pw.Widget> overlays,
      Map<String, dynamic> formData,
      Map<String, FieldPosition> positions,
      pw.Font font,
    ...
    // Determine which operation was performed and add text accordingly

    // Admiterea funcționării
    if (formData['operatia_admitere'] == true || formData['tip_aparat'] != null) {
      _addTextOverlay(
        overlays,
        'operatia_admitere',
        'Admitere',
  ...
  Map<String, FieldPosition> _getAnexa4FieldPositions() {
    return {
      'nr_inregistrare': FieldPosition(x: 722.6666666666636, y: 213.33333333333712, width: 404.66666666666674, height: 101.33333333333337,),
      'denumire_utilizator': FieldPosition(x: 722.6666666666631, y: 313.33333333333786, width: 404.66666666666674, height: 301.3333333333333,),
      'localitate_judet': FieldPosition(x: 723.999999999997, y: 613.3333333333375, width: 404.0, height: 126.00000000000011,),
      'strada_nr': FieldPosition(x: 723.999999999997, y: 736.666666666669, width: 402.66666666666674, height: 125.33333333333337,),
      'bl_sc_et_ap': FieldPosition(x: 723.3333333333305, y: 859.9999999999992, width: 403.33333333333326, height: 126.66666666666663,),
      'operatia_admitere': FieldPosition(x: 722.6666666666619, y: 985.3333333333148, width: 405.33333333333337, height: 142.66666666666674,),
      'operatia_vtp': FieldPosition(x: 722.6666666666619, y: 1127.9999999999816, width: 405.33333333333337, height: 126.66666666666652,),
      'operatia_reparare': FieldPosition(x: 722.6666666666619, y: 1252.666666666648, width: 405.33333333333337, height: 134.00000000000023,),
      'tip_aparat': FieldPosition(x: 723.3333333333305, y: 1385.333333333327, width: 404.66666666666674, height: 176.66666666666652,),
      'parametri_principali': FieldPosition(x: 723.3333333333305, y: 1558.6666666666565, width: 404.66666666666674, height: 204.0,),
      'nr_fabricatie_an': FieldPosition(x: 721.9999999999966, y: 1759.999999999986, width: 406.66666666666674, height: 202.66666666666674,),
      'producator_furnizor': FieldPosition(x: 723.3333333333305, y: 1959.9999999999825, width: 403.33333333333326, height: 226.66666666666674,),
      'raport_verificare_nr_data': FieldPosition(x: 723.3333333333305, y: 2184.6666666666533, width: 402.66666666666674, height: 202.66666666666697,),
      'livret_aparat_nr_data': FieldPosition(x: 723.3333333333305, y: 2386.666666666658, width: 404.0, height: 204.0,),
      'scadenta_urmatoare_verificare': FieldPosition(x: 722.666666666664, y: 2590.0, width: 405.33333333333326, height: 226.0,),
      'observatii': FieldPosition(x: 722.666666666664, y: 2814.0000000000095, width: 405.33333333333326, height: 203.33333333333348,),
    };
  }
}
